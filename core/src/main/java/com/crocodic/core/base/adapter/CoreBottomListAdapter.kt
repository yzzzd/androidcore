...
class CoreBottomListAdapter<T: Any?>(val items: List<T?>, val onItemClick: (data: T?) -> Unit) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        if (convertView == null) {
            val binding = DataBindingUtil.inflate<ItemBottomListBinding>(LayoutInflater.from(parent?.context), R.layout.item_bottom_list, parent, false)
    ...
    class ItemViewHolder<T: Any?>(val binding: ItemBottomListBinding) {
